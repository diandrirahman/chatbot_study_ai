...
const props = defineProps({ open: Boolean, history: { type: Array, required: true }, modelValue: { type: String, required: true }, error: { type: String, default: '' }, requestError: { type: String, default: '' }, submitting: Boolean, language: { type: String, default: 'en' } })
    ...
    <div v-if="open" class="drawer-layer" @keydown.esc="$emit('close')">
      ...
      <aside class="assistant-drawer" role="dialog" aria-modal="true" aria-labelledby="assistant-title">
        ...
        <div ref="messageList" class="drawer-messages" aria-live="polite">
          ...
          <article v-if="submitting" class="drawer-message drawer-assistant" role="status"><strong>StudyMate</strong><span class="typing-indicator"><i></i><i></i><i></i><span class="sr-only">{{ labels.responding }}</span></span></article>
