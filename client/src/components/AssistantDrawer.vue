...
const props = defineProps({
  ...
  pendingMessage: { type: String, default: '' },
...
const adjustmentHistory = () => props.history
...
watch(() => [props.open, props.history.length, props.pendingMessage, props.submitting], ([open]) => {
  if (open) void scrollToLatestMessage()
})
</script>
    ...
    <div v-if="open" class="drawer-layer" @keydown.esc="$emit('close')">
      ...
      <aside class="assistant-drawer" role="dialog" aria-modal="true" aria-labelledby="assistant-title">
        ...
        <div ref="messageList" class="drawer-messages" aria-live="polite">
          <div v-if="!adjustmentHistory().length && !pendingMessage" class="drawer-empty"><span aria-hidden="true">✦</span><h3>{{ labels.assistantEmptyTitle }}</h3><p>{{ labels.assistantEmptyDescription }}</p></div>
          ...
          <article v-if="pendingMessage" class="drawer-message drawer-user" aria-live="polite"><strong>{{ labels.you }}</strong><p>{{ pendingMessage }}</p></article>
